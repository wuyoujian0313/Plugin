...
iSlider.prototype._setting = function () {
    ...
    //default dot
    this.isPoint = opts.isPoint || false;
    //dot array
    this.aPoints=[];
...
/*creat dot*/
iSlider.prototype.creatDot=function(i)
{
    if(this.isPoint)
    {
...
iSlider.prototype._renderItem = function (el, i) {
    ...
    if (this.type === 'pic') {
        if (!this.isOverspread) {
            ...
            ? '<img height="' + this.height + '" src="' + item.imgSrc + '">'
            : '<img width="' + this.width + '" src="' + item.imgSrc + '">';
        } else {
            el.style.background = 'url(' + item.imgSrc + ') 50% 50% / cover no-repeat';
        }
    } 
...
iSlider.prototype._renderHTML = function () {
    ...
    if (!this.outer) {
        ...
        //append dot to div#canvas
    if(this.creatDot())
    {
        this.oDiv = document.createElement('div'); //放点
        var length=this.data.length;
        for(var i=0;i<length;i++)
        {
            var a= this.creatDot(i);
            this.oDiv.appendChild(a);
            this.aPoints.push(a);
        };
        this.pointActive(this.slideIndex);
        this.oDiv.className='mainDot';
...
iSlider.prototype._bindHandler = function() {
    ...
    outer.addEventListener('isliderTap', function(){self.tapHandler(self.slideIndex)});
...
//dot
iSlider.prototype.pointActive=function(index){
    var aPoint=this.aPoints;
